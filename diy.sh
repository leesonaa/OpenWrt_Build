...
sed -i '/sed -i "s\/# \/\/g" \/etc\/opkg\/distfeeds.conf/a\sed -i "\/openwrt_leesonaa\/d" \/etc\/opkg\/distfeeds.conf' package/lean/default-settings/files/zzz-default-settings
...
#sed -i 's/msgstr "概览"/msgstr "主页"/g' feeds/luci/modules/luci-mod-dashboard/po/zh_Hans/dashboard.po
